...
echo "RADICAL CACHE-BUSTING APPROACH"
echo "=================================================="
...
echo ""
echo "3. Creating new homepage with timestamp $timestamp..."

cat > /var/www/crane-intelligence/frontend/homepage.html << EOF
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
echo "OK: New homepage created with timestamp $timestamp"

# Step 4: Update Nginx with aggressive cache busting
echo ""
echo "4. Updating Nginx with radical cache busting..."

cat > /etc/nginx/sites-available/crane-intelligence << EOF
server {
    listen 80;
...
echo "OK: Nginx configuration updated with radical cache busting"
...
# Test the domain
echo "Testing domain..."
curl -s https://craneintelligence.tech | grep -i "$timestamp" || echo "WARN: Timestamp not found"
curl -s https://craneintelligence.tech | grep -i "auth.login" || echo "WARN: auth.login not found"
curl -s https://craneintelligence.tech | grep -i "NotificationSystem" || echo "WARN: NotificationSystem not found"

echo ""
echo "RADICAL CACHE-BUSTING COMPLETE!"
echo "Timestamp: $timestamp"
echo "Domain: https://craneintelligence.tech"
echo "Force refresh your browser (Ctrl+F5 or Cmd+Shift+R)"
