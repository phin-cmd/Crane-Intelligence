#!/usr/bin/env python3
"""
Script to fix report #21 payment status
...
def fix_report_21():
    """Fix report #21 payment status"""
    db = SessionLocal()
    try:
        # Find report #21
        report = db.query(FMVReport).filter(FMVReport.id == 21).first()
        if not report:
            logger.error("Report #21 not found")
            return False
        
        logger.info(f"Found report #21: status={report.status.value}, payment_status={report.payment_status}, amount_paid={report.amount_paid}, payment_intent_id={report.payment_intent_id}")
        
        # Payment intent ID: pi_3SfSi7BME5ZRi6sp3vKui23w
        if report.payment_intent_id:
            ...
            db.commit()
            db.refresh(report)
            logger.info(f"✅ Updated report #21: status={report.status.value}, amount_paid=${report.amount_paid}, payment_status={report.payment_status}")
            ...
            if user:
                ...
                logger.error("User not found for report #21")
            
            return True
        else:
            logger.warning("Report #21 has no payment_intent_id")
            return False
            
    except Exception as e:
        logger.error(f"Error fixing report #21: {e}", exc_info=True)
        db.rollback()
        return False
    finally:
        db.close()
...
if __name__ == "__main__":
    logger.info("Starting fix for report #21...")
    success = fix_report_21()
    if success:
        logger.info("✅ Report #21 fixed successfully!")
        sys.exit(0)
    else:
        logger.error("❌ Failed to fix report #21")
        sys.exit(1)
