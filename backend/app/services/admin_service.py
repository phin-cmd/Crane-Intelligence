...
class AdminService:
    ...
    async def get_dashboard_metrics(self, db: Session) -> DashboardMetrics:
        ...
        try:
            ...
            total_revenue = 0
            if basic_plan:
                total_revenue += basic_users * basic_plan.monthly_price
            if pro_plan:
                total_revenue += pro_users * pro_plan.monthly_price
            if enterprise_plan:
                total_revenue += enterprise_users * enterprise_plan.monthly_price
    ...
    async def get_user_analytics(self, db: Session, period: str) -> UserAnalytics:
        ...
        return UserAnalytics(
            ...
            user_engagement=[],
            subscription_distribution={"basic": 500, "pro": 200, "enterprise": 50},
            churn_rate=5.2,
            retention_rate=94.8
        )
